...
{
	// クリップ空間の座標に変換
	float4 worldViewProjectionPosition =
		mul(float4(worldPosition, 1), lightViewProjection);
...
// 返す値			: 影かどうか
float3	CalcShadowColor(
	Texture2D tex, SamplerState samplerState,
...
// shadowColor		: 影の色
// 返す値			: 影の色
float3	CalcShadowColorPCFFilter(
	Texture2D tex, SamplerState samplerState,
	float3 shadowTexcoord, float3 shadowColor, float shadowBias)
{
	// テクセルサイズの計算
	...
	for (int x = -PCFKernelSize / 2; x <= PCFKernelSize / 2; ++x)
	{
		...
		{
			// シャドウマップから深度値取得
			float depth = tex.Sample(
				samplerState, shadowTexcoord.xy + texelSize * float2(x, y)).r;
